    ...
    for col in scores_df.columns:
        ...
        if col in ['auc', 'pr_auc']:
            ...
            plot_consistently_false_trials_1d(
                ...
                savepath=os.path.join(save_path, f"consistent_false_trials_{col}.png")
            )
            ...
            # false trials that are filtered according to the max baseline
            df_filtered = filter_false_predictions_above_threshold(df_false, yvals, time_array, max_baseline)
            plot_false_predictions_with_baseline_bbox(
                df_filtered,
                ave_baseline,
                'max baseline',
                title='False predictions above max baseline score',
                savepath=os.path.join(save_path, f'False_predictions_ids_max_baseline_{col}_{subject_id}_session{session}.png')
            )
            export_trials_above_baseline(df_false, export_trials_path, file_name=f'False_predictions_ids_max_baseline_{col}_{subject_id}_session{session}')
...
def plot_results(savedir, scores_dic, roc_dic, pr_auc_dic, all_frames_confusion, all_frames_confusion_idx, subject_id, session, model, frame_bin, frame_rate):
    """
...
# con_path = "C:/Users/admin/RewardSizeDecoder pipeline/RewardSizeDecoder/results/hparams search- fps 5 Hz/Decoder LR output/464724/session1/pr_auc.pkl"
# with open(con_path, 'rb') as f:
#     pr_auc_dic = pickle.load(f)
#
# f_rate = 5
# df = extract_false_predictions(dic, f_rate)
# time_bin = [-10,50]
# time_array = np.arange(time_bin[0]*f_rate, time_bin[1]*f_rate+1)
# time = np.array([i / f_rate for i in time_array], dtype=float)
#
# threshold = 0.7
# consistent_trials = extract_consistently_false_trials(
#     df,
...
# plot_consistently_false_trials_1d(
#     consistent_trials_df=consistent_trials,
#     title=f"Consistently false predicted trials (≥{int(threshold*100)}% of time)",
#     savepath='C:/Users/admin/RewardSizeDecoder pipeline/RewardSizeDecoder/results/hparams search- fps 5 Hz/Decoder LR output/false_idx.png'
# )
#
# # plot_false_predictions_with_baseline_bbox(
# #     df,
